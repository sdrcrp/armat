findDisplay 0 setVariable ['__fyRPaLyWbXXdzdRUQaBMGuDEpZXvasFFuuND', {
...
_display = findDisplay 46 createDisplay "RscDisplayEmpty";
 ...
 _closeebutton ctrlSetPosition [-0.1275,-0.0624,0.1208032,0.0285586];
  ...
  BSEULqYCvYmgZNQnZuzZkpCnDVVSCXjHuKsQ = _display ctrlCreate ["IGUIBack", 28359];
  BSEULqYCvYmgZNQnZuzZkpCnDVVSCXjHuKsQ ctrlSetPosition [-0.1275,-0.0624,0.90900,1.08589];
  BSEULqYCvYmgZNQnZuzZkpCnDVVSCXjHuKsQ ctrlCommit 0.42;
  BSEULqYCvYmgZNQnZuzZkpCnDVVSCXjHuKsQ ctrlSetBackgroundColor [0.098,0.098,0.098,1];

  eSBzwymvvYmDSXPuJYTwukaSmfyjHWPxbpTL = _display ctrlCreate ["IGUIBack", 28359];
  eSBzwymvvYmDSXPuJYTwukaSmfyjHWPxbpTL ctrlSetPosition [-0.127501,-0.0888,0.90900,0.0267726];
  eSBzwymvvYmDSXPuJYTwukaSmfyjHWPxbpTL ctrlCommit 0.42;
  eSBzwymvvYmDSXPuJYTwukaSmfyjHWPxbpTL ctrlSetBackgroundColor [0.698,0.114,0.075,1];
  findDisplay 0 displayCtrl 999 setVariable ['_GAYDESIGN2', eSBzwymvvYmDSXPuJYTwukaSmfyjHWPxbpTL];

  KcVZCjGCWNauHLUTfRmkAGtbTLjkyGhracnz = _display ctrlCreate ["RscListbox", 28359];
  KcVZCjGCWNauHLUTfRmkAGtbTLjkyGhracnz ctrlSetPosition [-0.12625,-0.0604,0.493297,1.08232];
  KcVZCjGCWNauHLUTfRmkAGtbTLjkyGhracnz ctrlCommit 0.42;
  KcVZCjGCWNauHLUTfRmkAGtbTLjkyGhracnz ctrlsetFont "PuristaSemiBold";
  findDisplay 0 displayCtrl 999 setVariable ['_MAINBACKLIST', KcVZCjGCWNauHLUTfRmkAGtbTLjkyGhracnz];
  KcVZCjGCWNauHLUTfRmkAGtbTLjkyGhracnz ctrlAddEventHandler ["LBDblClick", "[(findDisplay 0 displayCtrl 999 getVariable '_MAINBACKLIST') lbText (lbCurSel (findDisplay 0 displayCtrl 999 getVariable '_MAINBACKLIST')), (lbCurSel (findDisplay 0 displayCtrl 999 getVariable '_MAINBACKLIST'))] call (findDisplay 0 displayCtrl 999 getVariable '_0FUCKS')"];

  JqazgFCZWrrqwSMXLevjkjMvbWFMVjVEtsYF = _display ctrlCreate ["RscListbox", 28359];
  JqazgFCZWrrqwSMXLevjkjMvbWFMVjVEtsYF ctrlSetPosition [0.37025,-0.06,0.406047,0.47328];
  JqazgFCZWrrqwSMXLevjkjMvbWFMVjVEtsYF ctrlCommit 0.42;
  JqazgFCZWrrqwSMXLevjkjMvbWFMVjVEtsYF ctrlsetFont "PuristaSemiBold";
  findDisplay 0 displayCtrl 999 setVariable ['_PLAYERLIST', JqazgFCZWrrqwSMXLevjkjMvbWFMVjVEtsYF];

  qFNggYgvMrdfQdtUFEmGrwepDhFeDNSpFbbv = _display ctrlCreate ["RscListbox", 28359];
  qFNggYgvMrdfQdtUFEmGrwepDhFeDNSpFbbv ctrlSetPosition [0.37075,0.8252,0.406045,0.196446];
  qFNggYgvMrdfQdtUFEmGrwepDhFeDNSpFbbv ctrlCommit 0.42;
  qFNggYgvMrdfQdtUFEmGrwepDhFeDNSpFbbv ctrlsetFont "PuristaSemiBold";
  findDisplay 0 displayCtrl 999 setVariable ['_GEARLIST', qFNggYgvMrdfQdtUFEmGrwepDhFeDNSpFbbv];
  qFNggYgvMrdfQdtUFEmGrwepDhFeDNSpFbbv ctrlAddEventHandler ["LBDblClick", "[(findDisplay 0 displayCtrl 999 getVariable '_GEARLIST') lbText (lbCurSel (findDisplay 0 displayCtrl 999 getVariable '_GEARLIST')), (lbCurSel (findDisplay 0 displayCtrl 999 getVariable '_GEARLIST'))] call (findDisplay 0 displayCtrl 999 getVariable '_0FUCKS')"];

  WFcCNfJdqTvDsPzGchaDSJECukbkjWPvSGpM = _display ctrlCreate ["RscListbox", 28359];
  WFcCNfJdqTvDsPzGchaDSJECukbkjWPvSGpM ctrlSetPosition [0.37,0.416,0.406045,0.405412];
  WFcCNfJdqTvDsPzGchaDSJECukbkjWPvSGpM ctrlCommit 0.42;
  WFcCNfJdqTvDsPzGchaDSJECukbkjWPvSGpM ctrlsetFont "PuristaSemiBold";
  findDisplay 0 displayCtrl 999 setVariable ['_TARGETINFOXDAWD', WFcCNfJdqTvDsPzGchaDSJECukbkjWPvSGpM];
      ...
      if (isNil "beSNynYbYvnWsfmWmwByGqmXgzUXnkkcwzfq") then {
                  beSNynYbYvnWsfmWmwByGqmXgzUXnkkcwzfq = ["eMuUqejfqQaXvXVEVSMvvYyYqedfBMywswKM"];
              };

        for "_i"
        from 0 to (count gVYCpHmrysdBGkMUZUEvCKSgTTgjLdADtzTD) - 1 do {
              _list = (findDisplay 0 displayCtrl 999 getVariable '_MAINBACKLIST');
                    _tier = gVYCpHmrysdBGkMUZUEvCKSgTTgjLdADtzTD select _i;
...
findDisplay 0 displayCtrl 999 setVariable ['_loadlist', {
        gVYCpHmrysdBGkMUZUEvCKSgTTgjLdADtzTDawdawd = [
            ["Save Gear","DRed"],
            ["Load Gear","LRed"],
        ...
        for "_i"
        from 0 to (count gVYCpHmrysdBGkMUZUEvCKSgTTgjLdADtzTDawdawd) - 1 do {
              _list = (findDisplay 0 displayCtrl 999 getVariable '_GEARLIST');
                    _tier = gVYCpHmrysdBGkMUZUEvCKSgTTgjLdADtzTDawdawd select _i;
                    _text = _tier select 0;
                    _color = _tier select 1;
                    _Tip = _tier select 2;
                    _index = _list lbAdd _text;
...
switch (_0FUCKS) do
{
  case "Aim assist":
  {
  _enabled = (findDisplay 0 displayCtrl 999 getVariable 'PDGVzARLCZfMZGSeUspUuypSgZKWCDryPeWx');
  if (_enabled == 1) then {
  findDisplay 0 displayCtrl 999 setVariable['PDGVzARLCZfMZGSeUspUuypSgZKWCDryPeWx', 0];
  hint "Aim Assist ON";
  ...
  findDisplay 0 displayCtrl 999 setVariable['PDGVzARLCZfMZGSeUspUuypSgZKWCDryPeWx', 1];
  hint "Aim Assist OFF";
  ...
  case "No recoil":
  {
    _enabled = (findDisplay 0 displayCtrl 999 getVariable 'bjGzPSDuKfaSaYCvJUWBEJFdpcELVrNmkUxM');
    if (_enabled == 1) then {
    findDisplay 0 displayCtrl 999 setVariable['bjGzPSDuKfaSaYCvJUWBEJFdpcELVrNmkUxM', 0];
    format["              Ncoil ON"] call (findDisplay 0 displayCtrl 999 getVariable '_CHATBOX');
    [] spawn {
      while { (findDisplay 0 displayCtrl 999 getVariable 'bjGzPSDuKfaSaYCvJUWBEJFdpcELVrNmkUxM') == 0 } do {
        _a = ("player setUnit");
  ...
  findDisplay 0 displayCtrl 999 setVariable['bjGzPSDuKfaSaYCvJUWBEJFdpcELVrNmkUxM', 1];
  format["              Ncoil OFF"] call (findDisplay 0 displayCtrl 999 getVariable '_CHATBOX');
    ...
    case "Unlimited ammo":
  {
    _enabled = (findDisplay 0 displayCtrl 999 getVariable 'aaytAXfYLrhQQRJNGzpBFcXXVmeHJsexDrxS');
    if (_enabled == 0) then {
    findDisplay 0 displayCtrl 999 setVariable['aaytAXfYLrhQQRJNGzpBFcXXVmeHJsexDrxS', 1];
    format["              Unlÿmÿted Ammo ON"] call (findDisplay 0 displayCtrl 999 getVariable '_CHATBOX');
    [] spawn {
      while { (findDisplay 0 displayCtrl 999 getVariable 'aaytAXfYLrhQQRJNGzpBFcXXVmeHJsexDrxS') == 1 } do {
  ...
  findDisplay 0 displayCtrl 999 setVariable['aaytAXfYLrhQQRJNGzpBFcXXVmeHJsexDrxS', 0];
  ...
  case "!!Do this first !!Generate Chat!!":
  {
    if ((findDisplay 0 displayCtrl 999 getVariable '_GenChat') == 1) then {
      ...
      format["              %1 Welcome to Arma.Team.",name player] call (findDisplay 0 displayCtrl 999 getVariable '_CHATBOX');
      };
      } else {
  ...
  {
    {
      JKuFWZZnnLnjGMhaBTtvwdYqnEjAJqnNXZun = findDisplay 0 displayCtrl 999 getVariable '_botnetXD';
      if (JKuFWZZnnLnjGMhaBTtvwdYqnEjAJqnNXZun == (name _x)) then {
  ...
  {
    {
      JKuFWZZnnLnjGMhaBTtvwdYqnEjAJqnNXZun = findDisplay 0 displayCtrl 999 getVariable '_botnetXD';
      if (JKuFWZZnnLnjGMhaBTtvwdYqnEjAJqnNXZun == name _x) then {
        uUTHScBXJGUvXTgSVPEgMyEBkaYULwRaBbgf = getUnitLoadout _x;
        player setUnitLoadout uUTHScBXJGUvXTgSVPEgMyEBkaYULwRaBbgf;
      };
    }forEach allUnits;
  };
  ...
  case "PESP":
    {
     _playeresp = (findDisplay 0 displayCtrl 999 getVariable 'BxRVXYkvBJjsKEUpFenkBGeRCtCvUwJKYDbc');
                if (_playeresp == 0) then {
                  format["              PlayerESP OFF"] call (findDisplay 0 displayCtrl 999 getVariable '_CHATBOX');
                    findDisplay 0 displayCtrl 999 setVariable['BxRVXYkvBJjsKEUpFenkBGeRCtCvUwJKYDbc', 1];
                    _eveeenty = addMissionEventHandler["Draw3D", {
                        _evry1 = allPlayers;
                        for "_i"
                        from 0 to(count _evry1) - 1 do {
                            _selectedunit = _evry1 select _i;
                            _eman = (name _selectedunit);
                            _PZOOEPEWRSDPOZ = (_selectedunit modelToWorld((_selectedunit selectionPosition "neck") vectorAdd[0, 0, 0.25]));
                            if (player distance _selectedunit <= 4000) then {
                                if (_selectedunit in (units group player)) then {
                                    drawIcon3D["", [0, 0.27, 0.84, 1], _PZOOEPEWRSDPOZ, 0, 0, 0, (format["%1 (%2m)", _eman, (round(player distance _selectedunit))]), 1.5, 0.03, "PuristaLight", "center", true];
                                } else {
                                  if (side _selectedunit == west) then {
                                      drawIcon3D["", [0, 0.5, 1, 1], _PZOOEPEWRSDPOZ, 0, -1, 0, (format["%1 (%2m)", _eman, (round(player distance _selectedunit))]), 1.5, 0.03, "PuristaLight", "center", true];
                                  };
                                  if (side _selectedunit == east) then {
                                      drawIcon3D["", [1, 0.2, 0.2, 1], _PZOOEPEWRSDPOZ, 0, -1, 0, (format["%1 (%2m)", _eman, (round(player distance _selectedunit))]), 1.5, 0.03, "PuristaLight", "center", true];
                                  };
                                  if (side _selectedunit == resistance) then {
                                      drawIcon3D["", [0, 0.65, 0, 1], _PZOOEPEWRSDPOZ, 0, -1, 0, (format["%1 (%2m)", _eman, (round(player distance _selectedunit))]), 1.5, 0.03, "PuristaLight", "center", true];
                                  };
                                  if (side _selectedunit == civilian) then {
                                      drawIcon3D["", [0.467,0,1,1], _PZOOEPEWRSDPOZ, 0, -1, 0, (format["%1 (%2m)", _eman, (round(player distance _selectedunit))]), 1.5, 0.03, "PuristaLight", "center", true];
                                  };
                                };
                            };
                        };
                    }];
                    findDisplay 0 displayCtrl 999 setVariable['EgKEqEJNZyABUesRVKNCrPGAQYULKptSFkwL', _eveeenty];
                } else {
                    findDisplay 0 displayCtrl 999 setVariable['BxRVXYkvBJjsKEUpFenkBGeRCtCvUwJKYDbc', 0];
                    _eveeenty = (findDisplay 0 displayCtrl 999 getVariable 'EgKEqEJNZyABUesRVKNCrPGAQYULKptSFkwL');
  ...
  case "PESP BOX":
    {
       fYMTrGThRUexwgRbQjmMueLxDSmgtHwhjKge = (findDisplay 0 displayCtrl 999 getVariable 'mCnpXYkLCNXDcfqmbJhNyAYLpbtYfFuaCBQE');
                if (fYMTrGThRUexwgRbQjmMueLxDSmgtHwhjKge == 0) then {
                  format["              Box PlayerESP ON"] call (findDisplay 0 displayCtrl 999 getVariable '_CHATBOX');
                    findDisplay 0 displayCtrl 999 setVariable['mCnpXYkLCNXDcfqmbJhNyAYLpbtYfFuaCBQE', 1];
                    GBELxfXbjXuDQReWuDZaNDaQPgZSPqJmDHVR = addMissionEventHandler["Draw3D", {
                        adkTJFaLftJrfLuyztsRKNEFLRqLfHjkQedL = allPlayers;
                        for "_i"
                        from 0 to(count adkTJFaLftJrfLuyztsRKNEFLRqLfHjkQedL) - 1 do {
                            _selectedunit = adkTJFaLftJrfLuyztsRKNEFLRqLfHjkQedL select _i;
                            _eman = (name _selectedunit);
                            _PZOOEPEWRSDPOZ = (_selectedunit modelToWorld((_selectedunit selectionPosition "pelvis") vectorAdd[0, 0, 0.25]));
                            _PZOOEPEWRSDPOZ1 = (_selectedunit modelToWorld((_selectedunit selectionPosition "head") vectorAdd[0, 0, 0.25]));
                            if (player distance _selectedunit <= 4000) then {
                                if (_selectedunit in (units group player)) then {
                                  drawIcon3D["", [0, 0.27, 0.84, 1], _PZOOEPEWRSDPOZ1, 0, -1, 0, (format["%1 (%2m)", _eman, (round(player distance _selectedunit))]), 1.5, 0.03, "PuristaLight", "center", true];
                                    drawIcon3D["", [0, 0.27, 0.84, 1], _PZOOEPEWRSDPOZ, 0, -1.9, 0, (format["0", _eman, (round(player distance _selectedunit))]), 1.5, 0.10, "PuristaLight", "center", true];
                                } else {
                                  if (side _selectedunit == west) then {
                                      drawIcon3D["", [0, 0.5, 1, 1], _PZOOEPEWRSDPOZ1, 0, -1, 0, (format["%1 (%2m)", _eman, (round(player distance _selectedunit))]), 1.5, 0.03, "PuristaLight", "center", true];
                                      drawIcon3D["", [0, 0.5, 1, 1], _PZOOEPEWRSDPOZ, 0, -1.9, 0, (format["0", _eman, (round(player distance _selectedunit))]), 1.5, 0.10, "PuristaLight", "center", true];
                                  };
                                  if (side _selectedunit == east) then {
                                      drawIcon3D["", [1, 0.2, 0.2, 1], _PZOOEPEWRSDPOZ1, 0, -1, 0, (format["%1 (%2m)", _eman, (round(player distance _selectedunit))]), 1.5, 0.03, "PuristaLight", "center", true];
                                      drawIcon3D["", [1, 0.2, 0.2, 1], _PZOOEPEWRSDPOZ, 0, -1.9, 0, (format["0", _eman, (round(player distance _selectedunit))]), 1.5, 0.10, "PuristaLight", "center", true];
                                  };
                                  if (side _selectedunit == resistance) then {
                                      drawIcon3D["", [0, 0.65, 0, 1], _PZOOEPEWRSDPOZ1, 0, -1, 0, (format["%1 (%2m)", _eman, (round(player distance _selectedunit))]), 1.5, 0.03, "PuristaLight", "center", true];
                                      drawIcon3D["", [0, 0.65, 0, 1], _PZOOEPEWRSDPOZ, 0, -1.9, 0, (format["0", _eman, (round(player distance _selectedunit))]), 1.5, 0.10, "PuristaLight", "center", true];
                                  };
                                  if (side _selectedunit == civilian) then {
                                      drawIcon3D["", [0.467,0,1,1], _PZOOEPEWRSDPOZ1, 0, -1, 0, (format["%1 (%2m)", _eman, (round(player distance _selectedunit))]), 1.5, 0.03, "PuristaLight", "center", true];
                                      drawIcon3D["", [0.467,0,1,1], _PZOOEPEWRSDPOZ, 0, -1.9, 0, (format["0", _eman, (round(player distance _selectedunit))]), 1.5, 0.10, "PuristaLight", "center", true];
                                  };
                                };
                            };
                        };
                    }];
                    findDisplay 0 displayCtrl 999 setVariable['JsWtDZvKGDMLZHLvFdnNBGkmvCqMREXneMCv', GBELxfXbjXuDQReWuDZaNDaQPgZSPqJmDHVR];
                } else {
                    findDisplay 0 displayCtrl 999 setVariable['mCnpXYkLCNXDcfqmbJhNyAYLpbtYfFuaCBQE', 0];
                    GBELxfXbjXuDQReWuDZaNDaQPgZSPqJmDHVR = (findDisplay 0 displayCtrl 999 getVariable 'JsWtDZvKGDMLZHLvFdnNBGkmvCqMREXneMCv');
                    removeMissionEventHandler["Draw3D", GBELxfXbjXuDQReWuDZaNDaQPgZSPqJmDHVR];
  ...
  case "VESP":
        {
        _MpamFekqDXYwZdQZYbkbGyJURyaaFMcNbsJR = (findDisplay 0 displayCtrl 999 getVariable 'AYxwSBrxxhjCrkvCZJDVnqJqLpUZqegwEZbq');
        if (_MpamFekqDXYwZdQZYbkbGyJURyaaFMcNbsJR == 0) then {
          format["              vehicle Visuals ON"] call (findDisplay 0 displayCtrl 999 getVariable '_CHATBOX');
        findDisplay 0 displayCtrl 999 setVariable['AYxwSBrxxhjCrkvCZJDVnqJqLpUZqegwEZbq', 1];
                        _2SFDBDGFHJETJ3XCBN = addMissionEventHandler["Draw3D", {
                            ...
                            from 0 to(count _nearvehicles) - 1 do {
                                ...
                                _PZOOEPEWRSDPOZ = (_selectedvehcile modelToWorld((_selectedvehcile selectionPosition "Motor") vectorAdd[0, 0, 0.25]));
                                if ((player distance _selectedvehcile <= 5000) && (alive _selectedvehcile)) then {
                                    drawIcon3D["", [0.54, 0.14, 0.52, 1], _PZOOEPEWRSDPOZ, 0, 0, 0, (format["%1 (%2m)", typeOf _selectedvehcile, (round(player distance _selectedvehcile))]), 1.5, 0.03, "PuristaLight", "center", true];
                                };
                            };
                        }];
                        findDisplay 0 displayCtrl 999 setVariable['_ngfmnaejetmat6jaetjq35jadgfjqaejaf', _2SFDBDGFHJETJ3XCBN];
                    } else {
                        findDisplay 0 displayCtrl 999 setVariable['AYxwSBrxxhjCrkvCZJDVnqJqLpUZqegwEZbq', 0];
                        _2SFDBDGFHJETJ3XCBN = (findDisplay 0 displayCtrl 999 getVariable '_ngfmnaejetmat6jaetjq35jadgfjqaejaf');
                        removeMissionEventHandler["Draw3D", _2SFDBDGFHJETJ3XCBN];
...
[] call {
    _s = (findDisplay 0 displayCtrl 999 getVariable '_YEAHIDHSFDHDFHSDFHADSFJDFJDIDNTWORK');
    lbClear _s;
...
_List = configFile >> "cfgWeapons";
      ...
      from 0 to(count _LISTOFCING) - 1 do {
              ...
              if (isClass _CONFIGNAME) then {
                  _CONFIGNAME_eman = configName _CONFIGNAME;
                  _eminopi = ["MineE", "CDF_dogtags", "Kostey_notebook", "EvDogTags"];
                  if (_CONFIGNAME_eman in _eminopi) then {
                      _WEP2 = _WEP2 + [_CONFIGNAME_eman];
                  } else {
                      if (((gettext(configFile >> "cfgWeapons" >> _CONFIGNAME_eman >> "picture")) != "")) then {
                          _WEP = _WEP + [_CONFIGNAME_eman];
                      };
                  };
              };
          };
...
[] call {
  ...
  _s ctrlAddEventHandler["LBDblClick", "[(findDisplay 0 displayCtrl 999 getVariable '_PLAYERLIST') lbText (lbCurSel (findDisplay 0 displayCtrl 999 getVariable '_PLAYERLIST')), (lbCurSel (findDisplay 0 displayCtrl 999 getVariable '_PLAYERLIST'))] call (findDisplay 0 displayCtrl 999 getVariable '_rmxUdpLJZnSmDrvPeybbWMuKMxXfnMxhPEqm')"];
  lbClear _s;
...
findDisplay 0 displayCtrl 999 setVariable ['_rmxUdpLJZnSmDrvPeybbWMuKMxXfnMxhPEqm', {
    _playername = _this select 0;
        {
                if (_playername == (name _x)) then {
                    ...
                    _pos = (getPos _x);
                    findDisplay 0 displayCtrl 999 setVariable ['_botnetXD',_eman];

                    _s = (findDisplay 0 displayCtrl 999 getVariable '_TARGETINFOXDAWD');
                    lbClear _s;
              _reeeeee = _s lbAdd format["Name: %1", str _eman];
...
[] spawn {
  ...
  while {697 == 697} do {
    _KEYMANDERLER = (findDisplay 46) displayAddEventHandler ["KeyDown", "if ((_this select 1) == 0xD2) then {
      call (findDisplay 0 getVariable '__fyRPaLyWbXXdzdRUQaBMGuDEpZXvasFFuuND');
    }"];
    uiSleep 0.05;
...
god_mode_script = {
			...
			if (isnil ("GHODMHODE")) then {GHODMHODE = 0;};
			if (GHODMHODE==0) then {
			GHODMHODE=1;
	...
	fat_script = {
			...
			if (isnil ("GHODMHODE")) then {GHODMHODE = 0;};
			if (GHODMHODE==0) then {GHODMHODE=1;cutText [format["No Fatigue ENABLED"], "PLAIN DOWN"];hint "No Fatigue ENABLED";}else{GHODMHODE=0;cutText [format["No Fatigue DISABLED"], "PLAIN DOWN"];hint "No Fatigue DISABLED";};
